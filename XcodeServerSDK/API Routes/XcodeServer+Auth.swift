...
extension XcodeServer {
       ...
       let _ =  self.sendRequestWithMethod(.post, endpoint: .login, params: nil, query: nil, body: nil) { (response, body, error) -> () in
            ...
            if let response = response as? HTTPURLResponse{
                if response.statusCode == 204 {
                    ...
                    completion(false, XcodeServerSDKError.wrongStatus(custom: "\(response.statusCode)"))
                }
                return
            }
            completion(false, XcodeServerSDKError.nilResponse)
        }
    }
       ...
       let _ =  self.sendRequestWithMethod(.get, endpoint: .userCanCreateBots, params: nil, query: nil, body: nil) { (response, body, error) -> () in
            ...
            if let body = body as? NSDictionary {
                if let canCreateBots = body["result"] as? Bool, canCreateBots == true {
                    ...
                    completion(false, XcodeServerSDKError.permission(custom: "Specified user cannot create bots"))
                }
            } else {
                completion(false, XcodeServerSDKError.wrongBody(custom: body as! String))
            }
        }
    }
    ...
    */
    public final func verifyXCSUserCanCreateBots(_ completion: @escaping (_ success: Bool, _ error: NSError?) -> ()) {
