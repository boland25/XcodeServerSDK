    ...
    final func postSCMBranchesWithBlueprint(_ blueprint: SourceControlBlueprint, completion: @escaping (_ response: SCMBranchesResponse) -> ()) {
        ...
        let _ = self.sendRequestWithMethod(.post, endpoint: .scm_Branches, params: nil, query: nil, body: blueprintDict) { (response, body, error) -> () in
            ...
            guard let responseObject = body as? NSDictionary else {
                let e = XcodeServerSDKError.wrongBody(custom: "\(body)")
                    ...
                    if let code = repoError["code"] as? Int {
                        ...
                        case -1004:
                            ...
                            if let fingerprint = repoError["fingerprint"] as? String {
                                ...
                                completion(XcodeServer.SCMBranchesResponse.error(XcodeServerSDKError.common(custom: "No fingerprint provided in error \(repoError)")))
                            }
                            
                        default:
                            completion(XcodeServer.SCMBranchesResponse.error(XcodeServerSDKError.common(custom: "Unrecognized error: \(repoError)")))
                        }
                    } else {
                        completion(XcodeServer.SCMBranchesResponse.error(XcodeServerSDKError.common(custom: "No code provided in error \(repoError)")))
                    }
                    return
            }
            ...
            guard
                ...
                let branches = branchesAllRepos[primaryRepoId] as? NSArray else {
                    ...
                    completion(XcodeServer.SCMBranchesResponse.error(XcodeServerSDKError.common(custom: "No branches provided for our primary repo id: \(primaryRepoId).")))
                    return
            }
            
            //cool, we gots ourselves some branches, let's parse 'em
            let parsedBranches = branches.map({ (name: ($0 as! [String: Any])["name"] as! String, isPrimary: ($0 as! [String: Any])["primary"] as! Bool) })
