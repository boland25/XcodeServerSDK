...
extension XcodeServer {
     ...
     */
    public final func getToolchains(_ completion: @escaping (_ toolchains: [Toolchain]?,_ error: NSError?) -> ()) {
       let _ = self.sendRequestWithMethod(.get, endpoint: .toolchains, params: nil, query: nil, body: nil) { (response, body, error) in
            ...
            if let body = (body as? NSDictionary)?["results"] as? NSArray {
                ...
                completion(nil, XcodeServerSDKError.wrongBody(custom:"Wrong body \(body)") as NSError?)
            }
        }
    }
}
